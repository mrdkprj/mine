...
const selectCell = (index: number, ignoreOpened = false) => {
    ...
    check(
        index,
        getRect(index).filter((i) => isValid(i)),
    );
};
...
const getRect = (index: number) => {
    ...
    if (poistion === POSITION.TopLeft) {
        return [index + 1, index + baseNumberOfCells, index + baseNumberOfCells + 1];
    }
    ...
    if (poistion === POSITION.FirstRow) {
        return [index - 1, index + 1, index + baseNumberOfCells - 1, index + baseNumberOfCells, index + baseNumberOfCells + 1];
    }

    if (poistion === POSITION.LastRow) {
        return [index - baseNumberOfCells - 1, index - baseNumberOfCells, index - baseNumberOfCells + 1, index - 1, index + 1];
    }

    if (poistion === POSITION.FirstColumn) {
        return [index - baseNumberOfCells, index - baseNumberOfCells + 1, index + 1, index + baseNumberOfCells, index + baseNumberOfCells + 1];
    }

    if (poistion === POSITION.LastColumn) {
        return [index - baseNumberOfCells - 1, index - baseNumberOfCells, index - 1, index + baseNumberOfCells - 1, index + baseNumberOfCells];
    }
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    await getCurrentWebviewWindow().setSize(new PhysicalSize(config.bounds.size.width, config.bounds.size.height));
    await getCurrentWebviewWindow().setPosition(new PhysicalPosition(config.bounds.position.x, config.bounds.position.y));
...
async function appready() {
    ...
    if (fileExists) {
        const rawData: string = await invoke("read_text_file", { payload: configFilePath });
        config = createConfig(JSON.parse(rawData));
    } else {
        config = defaultConfig;

        await invoke("write_text_file", { payload: { fullPath: configFilePath, data: JSON.stringify(defaultConfig) } });
    }
}
...
async function exists(target: string, createIfNotFound = false) {
    const result = await invoke("exists", { payload: target });

    if (!result && createIfNotFound) {
        await invoke("mkdir_all", { payload: target });
    }
...
async function writeConfig() {
    await invoke("write_text_file", { payload: { fullPath: configFilePath, data: JSON.stringify(config) } });
}

async function save() {
    config.isMaximized = await getCurrentWebviewWindow().isMaximized();
    const { width, height } = await getCurrentWebviewWindow().innerSize();
    const { x, y } = await getCurrentWebviewWindow().innerPosition();
